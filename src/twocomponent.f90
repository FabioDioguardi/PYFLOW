      subroutine twocomponent
        ...
        &usqmin, usqmax, cddmin, cddmax, cd2, d2mod, d2modm, s
         ...
         write (flog, 350) nmodel, cd1, cdd2, cdd100
...
305      format(/, 't tabulated =', f7.3, 2x, 't calculated =', f7.3,/)
         if (nrest .eq. 2) then
            write (*, 363) probt
            write (flog, 363) probt
            probt = probt - 0.1d0*probt
            alfa = probt/2.d0
            write (*, 364) probt
            write (flog, 364) probt
            goto 304
         else
            write (*, 365) probt
            write (flog, 365) probt
         end if
!     Model flow density
...
!     Normalized ushear^2
         ...
         if (usqnrm .ge. usq2 .or. usqnrm .le. usq100) then
            ...
            write (flog, *) 'Warning! ush^2 norm. is not in the range ush^2 (2 kg/m^3) - ush^2(100 kg/m^3)'
            write (flog, *) 'This is due to the very low accuracy level of the T - Student test'
