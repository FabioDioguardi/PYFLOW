...
!     START OF ITERATIONS
         reold = restart
         cdold = (24.d0/reold)*(((1.d0 - par)/reold + 1.d0)**exp_a) + (24.d0/reold)*0.1806d0*(reold**0.6459d0)* &
                 (par**(-(reold**exp_b))) + 0.4251d0/(1.d0 + 6880.95d0/(reold*(par**exp_c)))
         wtold = sqrt((4.d0*g*dpart*(rhop - rho))/(3.d0*cdold*rho))
50       renew = (rho*wtold*dpart)/mu
         cdnew = (24.d0/renew)*(((1.d0 - par)/renew + 1.d0)**exp_a) + (24.d0/renew)*0.1806d0*(renew**0.6459d0) &
                 *(par**(-(renew**exp_b))) + 0.4251d0/(1.d0 + 6880.95d0/(renew*(par**exp_c)))
         wtnew = sqrt((4.d0*g*dpart*(rhop - rho))/(3.d0*cdnew*rho))
         resre = abs(renew - reold)
         rescd = abs(cdnew - cdold)
         reswt = abs(wtnew - wtold)
